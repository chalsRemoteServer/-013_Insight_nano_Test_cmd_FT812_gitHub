...
void initStaticBackground(void){
 ...
 #if (TEST_UTF8 != 0) && (EVE_GEN > 2) //PROBAR ESTA PARTE
    ...
    EVE_cmd_text_bold(X_LabelParameter, Y_METAL_SIGNAL+GAP1*4, USER_FONT_SIZE, 0, "0s");
...
{   if(tft_active != 0) {
        ...
        if(EVE_IS_BUSY == EVE_busy()){return;} /* is EVE still processing the last display list? */
        ...
        switch(tag){
            ...
            case 10:if(0 == toggle_lock){ /* use button on top as on/off toggle-switch */
                    toggle_lock = 42;
                    if(0 == toggle_state){toggle_state = EVE_OPT_FLAT;}
                    else{toggle_state = 0;}}break;
            default:break;}}
}//+++++++++++++---------------------------------------------------------------------
...
{static int32_t rotate = 0;
    if(tft_active != 0U){//espera el init
